...
```{r}
library(tidyverse)
bar_seedlings <- health_assess_2025 %>% ggplot(aes(x = `Site Number or Initial: JC-W-_______`)) +
  geom_bar(aes(fill = `Site Number or Initial: JC-W-_______`)) + 
  xlab("2025 Sites Y/N")
...
## Producing seeds
```{r}
subset_seed_and_adults <- health_assess_2025 %>% filter(!is.na(producing_seed)) %>% filter(!is.na(dbh_cm))

print(subset_seed_and_adults %>% filter(producing_seed == "Yes") %>% select(plant_number))

bar_producing_seeds <- subset_seed_and_adults %>% ggplot(aes(x = producing_seed)) +
  geom_bar(aes(fill = producing_seed)) + 
  theme_bw() + 
  geom_text(stat = "count", aes(label = ..count..), vjust = -0.5, size = 8) +  # size 8 ≈ 24 pt
  ylim(0, 60) + 
  xlab("2025 Producing Seeds Y/N")
...
print(median_plant_height)

hist_plant_height <- health_assess_2025 %>% ggplot(aes(x = na.omit(plant_height_ft))) +
  geom_histogram(bins = 30, aes(fill = seedling_y_n)) +
  xlab("Height (ft)") + 
  ylab("Number of Individuals") +
  ggtitle("2025 All Individuals") + 
  # Mean
  geom_vline(
...
has_canker_sub <- health_assess_2025 %>% filter(has_canker == 'Yes')
count(has_canker_sub) # 114
count(health_assess_2025) # 146
